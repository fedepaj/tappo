...
gpio_t alert_pin = GPIO_PIN(PORT_B, 5); //D4
gpio_t led_pin = GPIO_PIN(PORT_B,4); //D5
gpio_t operative_pin = GPIO_PIN(PORT_A, 2); //D1
...
void genTimeStamp(char* stmp_str){
    timex_t stmp = timex_set(3,3);
    timex_to_str(stmp, stmp_str);
}
...
void switch_cb(void *arg){
    ...
    if(val){
        ...
        genTimeStamp(stmp_str);
        sprintf(str, "{\"id\":\"%s\",\"stmp\":\"%s\",\"active\":\"1\"}}",EMCUTE_ID, stmp_str);
        pub(MQTT_TOPIC_OUT, str, 0);
    } else {
        onTop=0;
        perc=0;
        genTimeStamp(stmp_str);
        sprintf(str, "{\"id\":\"%s\",\"stmp\":\"%s\",\"active\":\"0\"}}",EMCUTE_ID, stmp_str);
        pub(MQTT_TOPIC_OUT, str, 0);
    }
...
int main(void){
    ...
    while(1) {
        if(onTop){
            ...
            if(perc>=0){
                sprintf(str, "{\"id\":\"%s\",\"stmp\":\"%s\",\"perc\":\"%d\"}", EMCUTE_ID, stmp_str, perc);
                pub(MQTT_TOPIC_OUT, str, 0);
                if(perc>=100){
                    make_sound();
                    gpio_set(led_pin);
